...
/// </summary>
public static partial class TelegramUtils
{
    [Pure]
    public static string EscapeHtml(this string src)
    {
    ...
    /// Either <see cref="ParseMode.MarkdownV2"/> or <see cref="ParseMode"/>.
    /// Defaults to <see cref="ParseMode"/>
    /// </param>
    /// <param name="entityType">
    /// For the entity types <see cref="MessageEntityType"/>, <see cref="MessageEntityType"/> and
    /// the link part of <see cref="MessageEntityType"/>, only certain characters
    /// need to be escaped in <see cref="ParseMode"/>.
    /// See the official API documentation for details. Only valid in combination with
    /// <paramref name="parseMode"/>: <see cref="ParseMode"/>, will be ignored else.
    /// </param>
    /// <returns>The escaped text.</returns>
    /// <exception cref="Markdown">Only <see cref="ParseMode.Markdown"/> and <see cref="ParseMode.MarkdownV2"/> allowed.</exception>
        ...
        bool group = false)
    {
        ...
        {
            return DeepLinkPayloadRegex().IsMatch(input: payload);
        }
    }
